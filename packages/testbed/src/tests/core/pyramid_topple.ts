        ...
        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n - i; j++) {
                ...
                if (j === 0) {
                    add_domino(
                        world,
                        b2.Vec2.AddVV(offset, new b2.Vec2(0.5 * (WIDTH - HEIGHT), HEIGHT + WIDTH), new b2.Vec2()),
                        false
                    );
                }
                ...
                if (j !== n - i - 1) {
                    add_domino(
                        world,
                        b2.Vec2.AddVV(offset, new b2.Vec2(HEIGHT * 0.75, (HEIGHT + 3 * WIDTH) / 2), new b2.Vec2()),
                        true
                    );
                } else {
                    add_domino(
                        world,
                        b2.Vec2.AddVV(offset, new b2.Vec2(0.5 * (HEIGHT - WIDTH), HEIGHT + WIDTH), new b2.Vec2()),
                        false
                    );
                }
            }
